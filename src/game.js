...
export default class Game {
  constructor(gameWidth, gameHeight) {
    ...
    document.addEventListener("keydown", event => {
      ...
      switch (event.keyCode) {
        case 32:
          this.bullet = new Bullet(this, this.car, this.carArray);
      }
    });
  }
  ...
  update(deltaTime, x) {
    if (this.gameState === GAMESTATE.RUNNING) {
      this.carArray.forEach(car => car.update(deltaTime));
      [...this.gameOjects].forEach(object => object.update(deltaTime));
      ...
      if (this.bullet !== undefined) {
        this.bullet.update(deltaTime, this.carArray);
      }
    }
  }
